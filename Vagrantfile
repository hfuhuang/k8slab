...
$ssh_without_password_script = <<SCRIPT
#check for private key for vm-vm comm
[ -f /vagrant/inventory/local/ssh_key/id_rsa ] || {
  ssh-keygen -t rsa -f /vagrant/inventory/local/ssh_key/id_rsa -q -N ''
}
#deploy key
[ -f /home/vagrant/.ssh/id_rsa ] || {
    cp /vagrant/inventory/local/ssh_key/id_rsa /home/vagrant/.ssh/id_rsa
    chmod 0600 /home/vagrant/.ssh/id_rsa
}
#allow ssh passwordless
grep 'vagrant@k8s' ~/.ssh/authorized_keys &>/dev/null || {
  cat /vagrant/inventory/local/ssh_key/id_rsa.pub >> ~/.ssh/authorized_keys
...
if !Vagrant.has_plugin?("vagrant-hostmanager")
    ...
    puts "vagrant-hostmanager: Already installed "
end
...
Vagrant.configure("2") do |config|
  ...
  (1..$num_instances+1).each do |i|
    ...
    config.vm.define vm_name = "%s-%01d" % [$instance_name_prefix, i] do |node|
      ...
      node.vm.provision "shell", privileged: false, inline: $ssh_without_password_script
      ...
      if i > $num_instances
        # port forwarding for kubectl proxy on the ansible controller
        node.vm.network "forwarded_port", guest: 8001, host: 8001, auto_correct: true
        # resize the disk size to have nfs storage on the ansible controller
        node.vm.provision "shell", inline: "sudo xfs_growfs -d /"
        node.vm.provision "shell", path: "scripts/ansible_controller_provision_script.sh"
